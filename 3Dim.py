    ...
    def __call__(self, batch: dict[str, jnp.ndarray], cond_mask: jnp.ndarray, train: bool):
        ...
        for i_level in self.ch_mult:
            ...
            for i_block in range(self.num_res_blocks):
                # first two layers has no attention
...
if __name__ == "__main__":
    ...
    # Create the tensor for the noisy input image

    # Create the key-value pairs in the dictionary
    # breakpoint()
    rng = random.PRNGKey(1701)
    z = random.normal(rng, shape=(1, 64, 64, 3))
    x = random.normal(rng, shape=(1,64, 64, 3))
    logsnr = random.normal(rng, shape=(1,))
    t = random.normal(rng, shape=(1,2, 3))
    R = random.normal(rng, shape=(1,2, 3, 3))
    ...
    # X(batch=data,  cond_mask= jnp.array([1]), train=True)